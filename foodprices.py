...
# -----------------------------------------------------------------------------
# 2. GLOBAL DATA DICTIONARIES & CONSTANTS (DECLARED BEFORE SIDEBAR)
# -----------------------------------------------------------------------------
COMMODITIES = {
    "Crude Oil (WTI - Energy)": {"ticker": "CL=F", "unit": "per barrel", "exchange": "NYMEX", "currency": "USD"},
    "Wheat (Agriculture)": {"ticker": "ZW=F", "unit": "per bushel", "exchange": "CBOT", "currency": "US Cents (Div by 100)"},
    "Corn (Agriculture)": {"ticker": "ZC=F", "unit": "per bushel", "exchange": "CBOT", "currency": "US Cents (Div by 100)"},
    "Coffee (Arabica)": {"ticker": "KC=F", "unit": "per lb", "exchange": "ICE", "currency": "US Cents (Div by 100)"},
    "Sugar #11": {"ticker": "SB=F", "unit": "per lb", "exchange": "ICE", "currency": "US Cents (Div by 100)"},
    "Gold (Precious Metal)": {"ticker": "GC=F", "unit": "per troy oz", "exchange": "COMEX", "currency": "USD"}
}

FX_CURRENCIES = {
    "United States (USD)": {"iso": "USD", "symbol": "$", "fx_ticker": None, "source": "Base Currency"},
    "Eurozone (EUR)": {"iso": "EUR", "symbol": "€", "fx_ticker": "EURUSD=X", "source": "Yahoo Finance Spot FX"},
    "United Kingdom (GBP)": {"iso": "GBP", "symbol": "£", "fx_ticker": "GBPUSD=X", "source": "Yahoo Finance Spot FX"},
    "Japan (JPY)": {"iso": "JPY", "symbol": "¥", "fx_ticker": "JPY=X", "source": "Yahoo Finance Spot FX"},
    "India (INR)": {"iso": "INR", "symbol": "₹", "fx_ticker": "INR=X", "source": "Yahoo Finance Spot FX"},
    "Canada (CAD)": {"iso": "CAD", "symbol": "CA$", "fx_ticker": "CAD=X", "source": "Yahoo Finance Spot FX"}
}
...
# Comprehensive Global CSS Theme Overrides
if selected_theme == "Light":
    ...
    st.markdown(
        ...
        <style>
            ...
            /* 4. Dropdowns / Selectboxes Fix (Input + Popover List) */
            ...
            /* 5. Code Blocks & Copy Icon */
            [data-testid="stCodeBlock"], pre {
                background-color: #F1F5F9 !important;
                border: 1px solid #CBD5E1 !important;
                border-radius: 6px !important;
            }
            [data-testid="stCodeBlock"] *, [data-testid="stCodeBlock"] code, pre code, [data-testid="stCodeBlock"] span {
                color: #0F172A !important;
            ...
            [data-testid="stCodeBlock"] button svg, [data-testid="stCodeBlock"] button path {
                fill: #0F172A !important;
                stroke: #0F172A !important;
                color: #0F172A !important;
            }

            /* 6. Inline Code Badges */
            ...
            /* 9. Action Buttons & Download Button */
...
# -----------------------------------------------------------------------------
# 4. DATA FETCHING & FORECAST ENGINE WITH INFLATION FLOOR
# -----------------------------------------------------------------------------
@st.cache_data(ttl=3600)
def fetch_and_forecast_commodity(ticker_symbol, target_year, fx_ticker, baseline_inflation_rate):
    ...
    # Unit conversion for futures quoted in cents
    ...
    # Macro Inflation Floor Constraint
    ...
    forecast['inflation_floor'] = latest_converted_price * ((1.0 + baseline_inflation_rate) ** forecast['years_ahead'])
...
st.markdown("Commodity prices (from 2010) with dynamic machine learning projections up to **2046**.")
...
if df_data is not None:
    ...
    with tab_forecast:
        ...
        fig.add_trace(go.Scatter(
            x=hist_df['ds'], y=hist_df['price_converted'],
            mode='lines', name='Historical Data (From 2010)',
        ...
        if not forecast_df.empty:
            ...
            fig.add_trace(go.Scatter(
                x=plot_forecast['ds'], y=plot_forecast['price_converted'],
                mode='lines', name=f'Meta Prophet Forecast (to {selected_horizon_year})',
        ...
        fig.update_xaxes(
            title_text="Date", color=chart_text_color,
            tickfont=dict(color=chart_text_color, size=11), title_font=dict(color=chart_text_color, size=13),
            showgrid=True, gridcolor=grid_color, zeroline=False
        )
        fig.update_yaxes(
            title_text=f"Price ({country_info['symbol']} {country_info['iso']})", color=chart_text_color,
            tickfont=dict(color=chart_text_color, size=11), title_font=dict(color=chart_text_color, size=13),
            showgrid=True, gridcolor=grid_color, zeroline=False
        )
        ...
        st.subheader(f"📊 Projected Annual Averages ({country_info['iso']})")
        yearly_summary = []
        ...
        st.dataframe(pd.DataFrame(yearly_summary), use_container_width=True, height=350)

    # =========================================================================
    # TAB 2: DATA PIPELINE & VALUATION ENGINE (EXPANDED DATA SOURCES & EXTRACTION)
    # =========================================================================
    with tab_engine:
        ...
        # SECTION 1: DATA SOURCES & METADATA TABLE
        st.subheader("📡 1. Integrated Data Sources & Exchange Metadata")
        st.markdown("Primary market quotes are dynamically pulled using the `yfinance` API layer targeting continuous futures contracts.")
        ...
        for f_name, f_data in FX_CURRENCIES.items():
            fx_source_rows.append({
                ...
                "FX Spot Ticker": f_data["fx_ticker"] if f_data["fx_ticker"] else "N/A (Base)",
        ...
        # SECTION 2: EXTRACTION & PROCESSING METHODOLOGY
        ...
        # SECTION 3: INSPECTION TABLE & CSV EXPORT
    ...
    interval_width=0.80
        ...
        dataset_specs = [
            ...
            {"Attribute": "Target Forecast Horizon", "Specification": "Up to Year 2046", "Details": "Dynamic multi-year forward projection horizon"}
        ]
